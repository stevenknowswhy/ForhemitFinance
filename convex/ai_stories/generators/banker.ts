...
export const _generateBankerStoryInternal = internalAction({
  ...
  handler: async (ctx, args) => {
    try {
      ...
      const fallbackPrompt = STORY_SYSTEM_PROMPTS.banker[args.periodType === "annually" ? "quarterly" : args.periodType] || STORY_SYSTEM_PROMPTS.banker.monthly;
      ...
      // TODO: Send notification (needs to be scheduled as an internal mutation)
...
export const generateBankerStory = action({
  ...
  handler: async (ctx, args): Promise<{ storyId: Id<"ai_stories">; success: boolean; error?: string }> => {
    try {
      ...
      // @ts-ignore - TypeScript recursion limit, runtime is correct
      await ctx.scheduler.runAfter(0, internal.ai_stories.generators.banker._generateBankerStoryInternal, {
        storyId,
        userId: user._id,
        periodStart: args.periodStart,
        periodEnd: args.periodEnd,
