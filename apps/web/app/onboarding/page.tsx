...
// Hardcoded fallback business types (used only if Convex query fails)
...
export default function OnboardingPage() {
  ...
  // GAP-002: Fetch business types from Convex instead of hardcoded constant
  const businessTypesFromConvex = useQuery(api.businessTypes.getActiveBusinessTypes);
  const businessTypes = businessTypesFromConvex ?? FALLBACK_BUSINESS_TYPES;
  const [justCompleted, setJustCompleted] = useState(false);
  const [completedOrgId, setCompletedOrgId] = useState<string | null>(null);
  const { currentOrgId: orgId, setCurrentOrg } = useOrg();
